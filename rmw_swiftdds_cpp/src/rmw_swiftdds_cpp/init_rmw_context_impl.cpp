...
static rmw_ret_t init_context_impl(rmw_context_t *context)
{
  ...
  participant_info(
    rmw_swiftdds_shared_cpp::create_participant(greenstone_swiftdds_identifier,
    ...
    [&](CustomParticipantInfo *participant_info) {
      if(RMW_RET_OK != rmw_swiftdds_shared_cpp::destroy_participant(participant_info)) {
        RCUTILS_SAFE_FWRITE_TO_STDERR("Failed to destroy participant after function: "
  ...
  std::unique_ptr<rmw_publisher_t, std::function<void(rmw_publisher_t *)>> publisher(
    ...
    [&](rmw_publisher_t *pub) {
      ...
      {
        RCUTILS_SAFE_FWRITE_TO_STDERR("Failed to destroy publisher after function: "
                                        "'" RCUTILS_STRINGIFY(__function__) "' failed.\n");
      }
    });
  if(!publisher) {
  ...
  std::unique_ptr<rmw_subscription_t, std::function<void(rmw_subscription_t *)>> subscription(
    ...
    [&](rmw_subscription_t *sub) {
      ...
      {
        RMW_SAFE_FWRITE_TO_STDERR("Failed to destroy subscription after function: "
                                    "'" RCUTILS_STRINGIFY(__function__) "' failed.\n");
      }
    });
  if(!subscription) {
  ...
  graph_guard_condition(
    ...
    [&](rmw_guard_condition_t *p) {
      ...
      if(ret != RMW_RET_OK) {
        RMW_SAFE_FWRITE_TO_STDERR("Failed to destroy guard condition after function: "
                                        "'" RCUTILS_STRINGIFY(__function__) "' failed.\n");
      }
    });
  ...
  common_context->publish_callback = [](const rmw_publisher_t *pub, const void *msg) {
      return rmw_swiftdds_shared_cpp::__rmw_publish(
        greenstone_swiftdds_identifier, pub, msg, nullptr);
    };
  ...
  common_context->graph_cache.set_on_change_callback(
    [guard_condition = graph_guard_condition.get()]() {
      rmw_swiftdds_shared_cpp::__rmw_trigger_guard_condition(greenstone_swiftdds_identifier,
                                                               guard_condition);
      });
