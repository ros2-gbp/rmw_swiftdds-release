...
void node_listener(rmw_context_t *context)
{
  ...
  while(common_context->thread_is_running.load()) {
    ...
    if(RMW_RET_OK != rmw_swiftdds_shared_cpp::__rmw_wait(context->implementation_identifier,
                                                         &subscriptions,
                                                         &guard_conditions,
                                                         nullptr,
                                                         nullptr,
    ...
    if(subscriptions_buffer[0]) {
      ...
      while(taken) {
        if(RMW_RET_OK != rmw_swiftdds_shared_cpp::__rmw_take(context->implementation_identifier,
                                                             common_context->sub,
        ...
        if(taken) {
          if(std::memcmp(reinterpret_cast<char *>(common_context->gid.data),
