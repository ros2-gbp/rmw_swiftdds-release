...
__rmw_create_wait_set(const char *identifier, rmw_context_t *context, size_t max_conditions)
{
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(init context,
  ...
  RMW_TRY_PLACEMENT_NEW(swiftdds_wait_set,
                        wait_set->data,
                        goto fail,
...
rmw_ret_t __rmw_destroy_wait_set(const char *identifier, rmw_wait_set_t *wait_set)
{
  ...
  if(wait_set->data) {
    if(swiftdds_wait_set) {
      RMW_TRY_DESTRUCTOR(swiftdds_wait_set->greenstone::dds::WaitSet::~WaitSet(),
                         swiftdds_wait_set,
